...
module.exports = {
    name: "setantispam",
    description: "Configure antispam system",
    ...
    options: [
        {
            type: "string",
            name: "status",
            description: "Antispam status (On/Off)",
            required: true,
            autocomplete: true
        }
    ],
    async run(bot, message, args, db) {
        ...
        if(etat === "off") {
            ...
            db.query(`UPDATE server SET antispam = 'false' WHERE guild = '${message.guild.id}'`)
              const btn2 = new Discord.ActionRowBuilder().addComponents(new Discord.ButtonBuilder()
        	.setCustomId("AsDis")
        	.setLabel("Antispam has been disabled!")
        	.setStyle(Discord.ButtonStyle.Danger)
        	.setDisabled(true))
            ...
            db.query(`UPDATE server SET antispam = 'true' WHERE guild = '${message.guild.id}'`)
              const btn3 = new Discord.ActionRowBuilder().addComponents(new Discord.ButtonBuilder()
        	.setCustomId("ind")
        	.setLabel("Antispam has been enabled!")
